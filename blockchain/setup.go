...
func (setup *FabricSetup) Initialize() error {
	...
	req := resmgmt.SaveChannelRequest{ChannelID: setup.NewChannelID, ChannelConfigPath: setup.ChannelConfig, SigningIdentities: []msp.SigningIdentity{adminIdentity}}
	...
	if err = setup.admin.JoinChannel(setup.NewChannelID, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint(setup.OrdererID)); err != nil {
		return errors.WithMessage(err, "failed to make admin join channel")
	}
	fmt.Println("Channel joined")

	resp, err := setup.admin.QueryChannels(resmgmt.WithTargetEndpoints("peer0.LotusOrg1.lotus.com"))
	if err != nil {
		fmt.Println("Failed to query joined channels: ", err)
	}
	fmt.Println("Joined channels: ", resp.GetChannels())
...
func (setup *FabricSetup) InstallAndInstantiateCC() error {
	...
	ccPolicy := cauthdsl.SignedByAnyMember([]string{"LotusOrg1.lotus.com"})
